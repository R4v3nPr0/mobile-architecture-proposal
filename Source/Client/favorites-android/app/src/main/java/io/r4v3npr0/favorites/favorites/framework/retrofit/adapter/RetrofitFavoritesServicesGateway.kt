...
class RetrofitFavoritesServicesGateway: FavoritesServicesGateway {
    ...
    override fun addFavorite(favorite: FavoriteModel): Result<String, Throwable> {
        val call = favoritesApi.addFavorite(FavoritesConverter.convertToAddFavoriteRequestModel(favorite))

        return try {
            val response = call.execute()

            if (response.isSuccessful) {
                val id = response.body()?.favorite?.id
